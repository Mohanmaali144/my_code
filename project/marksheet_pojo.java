...
class Pojo
{
   ...
   public Pojo(String name,String course,String fname,String mname,String branch,String uv,int sem,int age,int roll,int year,int physics,int chemistry,int math,int hindi,int english,int total, double per )
   {
       this. name = name;
       this.course = course;
       this.fname = fname;
   ...
   //input Details
	public static void input(){
		...
		{	
			...
			per = total/5;
		    std[i] = new Pojo(name,course,fname,mname,branch,uv,sem,age,roll, year, physics,chemistry,math,hindi,english,total,per);
			i++;
		}			
		System.out.println("||-------------------------------------------------------------||\n");
		op.search(std,n);
